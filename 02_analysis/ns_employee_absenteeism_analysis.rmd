...
## Is the Difference in Counts Absences by Gender Significant?

Instead of doing some time series forecasting, as in the previous article, I thought I would mix it up and test to see if the counts of absences by gender is significant.

First, here is a look at the count of absences by gender: 
...
tidy_data.groupby(['gender'])['absence_date'].count().plot(kind = 'barh',figsize = (7,6),color = ('#4C586F'))
plt.title('Count of Absences by Gender')

plt.show()
```

Females have taken approximately 700,700 more absences than their male counterparts, but again, we do not know the employee counts. An interesting note, when I did completed this visual, only two genders were listed. Now, there is an "undefined" category with 116 total absences.
```{python}
# tidy_data['gender'].value_counts()
```
